...
A single data record can by retrieved by the --index (shortted as -i) flag.
...
diretory. Separate small files are named after unique column value. 
...
syntaxes like -c 0,1,2 or -c 0-2, including 2.
...
Options:
  -s, --sep <SEP>        Separator [default: ,]
      --no-header        Whether the file has a header
  -c, --cols <COLS>      Columns to select, support syntax 0,1,3 or 0-4, including 4; Default to select all columns [default: ]
  -f, --filter <FILTER>  Row filter, support syntax 0=a,b,c or 0=a,b&1=c,d; Default to None [default: ]
